"""Deep-dive Chilled comparison: NEW (v4.2 warm-start, PROD candidate) vs PRODUCTION.

Mirrors deep_dive_ambient.py but for the Chilled zone. Chilled is both a
cold_chain_zone (HC12 30-min cap, _split_for_cold_chain fires inside v2's
trolley builder) and an affinity_zone (Phase B v4.2 warm-start applies on top).

Run:
    python deep_dive_chilled.py
"""
...
def main() -> None:
    ...
    new_tote_items = [
        [(v2.aisle_int(it.aisle_location), v2.bay_int(it.bay_location), it.transit_id, it.stock_code, it.quantity)
         for it in t.items]
        for t in new_totes
    ]
