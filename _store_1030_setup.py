"""Store 1030 setup shim — side-effect import.

Monkey-patches v2 + v4 with 1030-specific paths and StoreConfig:
  - PAT cap = 2 (pick_across_trucks=2)
  - per-tote volume cap = 45L (capacity_max_volume_cm3=45000)
  - two staging locations (end_anchor_alt left at default "staging_location_2")

On first run, converts "1030 .xlsx" -> 1030Orders.csv in the repo root.
Subsequent runs reuse the cached CSV.

Usage:
    import _store_1030_setup  # must be first import in the script
"""
...
_ORDERS_CSV = os.path.join(THIS_DIR, "1030Orders.csv")
_DIST_MATRIX_CSV = os.path.join(THIS_DIR, "1030_dist_mat_onlineaisles_core.csv")


def _ensure_orders_csv() -> None:
    if os.path.exists(_ORDERS_CSV):
        return
    src = os.path.join(THIS_DIR, "1030 .xlsx")  # note trailing space in filename
    ...
    print(f"[setup-1030] Converting {src} -> {_ORDERS_CSV} ...", flush=True)
    ...
    wb.close()
    print(f"[setup-1030] Done.", flush=True)
...
# Patch StoreConfig — replace with a factory that returns a 1030-configured instance
_OrigStoreConfig = v2.StoreConfig


def _StoreConfig1030():
    ...
    # end_anchor_alt stays at default "staging_location_2" (1030 has two staging locations)
    return cfg


v2.StoreConfig = _StoreConfig1030
v4.StoreConfig = _StoreConfig1030
